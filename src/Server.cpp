...
namespace pex {
	Server::Server(std::function<void(const std::string&)> onStringMessage)
	...
	void Server::onMessage(websocketpp::connection_hdl, WebppServer::message_ptr messagePtr)
	{
		onStringMessage_(messagePtr->get_payload());
	}
} // pex